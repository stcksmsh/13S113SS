...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      // "expression"
      // expression
      char dummy1[sizeof (Assembler::EquStruct*)];

      // "control and status register"
      char dummy2[sizeof (Assembler::csr_type)];

      // "operand"
      // operand
      char dummy3[sizeof (Assembler::extended_instruction)];
      ...
      // symbol_or_literal_list
      char dummy7[sizeof (std::vector<uint32_t>)];
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_expression: // expression
        value.move< Assembler::EquStruct* > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_operand: // operand
        value.move< Assembler::extended_instruction > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Assembler::EquStruct*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Assembler::EquStruct*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Assembler::extended_instruction&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Assembler::extended_instruction& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      case symbol_kind::S_53_expression_: // "expression"
      case symbol_kind::S_expression: // expression
        value.template destroy< Assembler::EquStruct* > ();
        break;
      ...
      case symbol_kind::S_52_operand_: // "operand"
      case symbol_kind::S_operand: // operand
        value.template destroy< Assembler::extended_instruction > ();
        break;
...
#if 201103L <= YY_CPLUSPLUS
      symbol_type (int tok, Assembler::EquStruct* v, location_type l)
        : super_type (token_kind_type (tok), std::move (v), std::move (l))
#else
      symbol_type (int tok, const Assembler::EquStruct*& v, const location_type& l)
...
#if 201103L <= YY_CPLUSPLUS
      symbol_type (int tok, Assembler::extended_instruction v, location_type l)
        : super_type (token_kind_type (tok), std::move (v), std::move (l))
#else
      symbol_type (int tok, const Assembler::extended_instruction& v, const location_type& l)
        : super_type (token_kind_type (tok), v, l)
#endif
      {
#if !defined _MSC_VER || defined __clang__
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_NUMBER (int32_t v, location_type l)
      {
      ...
      static
      symbol_type
      make_NUMBER (const int32_t& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_GPR (uint8_t v, location_type l)
      {
      ...
      static
      symbol_type
      make_GPR (const uint8_t& v, const location_type& l)
      {
