...
std::istream &
ELF::deserialize(std::istream &stream)
{

    /* First we have to read the file */
    /// First we read the ELF header
    ELF_Header new_file_header;
    std::vector<Program_Header> new_program_headers;
    std::vector<uint8_t> new_binary_data;
    std::vector<Section_Header> new_section_headers;
    std::vector<std::string> new_section_names;
    { /// Read the ELF file
        stream.read((char *)&new_file_header, sizeof(ELF_Header));
        ...
        for (int i = 0; i < file_header.ph_num; i++)
        {
            ...
            new_program_headers.push_back(ph);
        }

        /// Then we read the binary data
        new_binary_data.resize(file_header.sh_offset - (sizeof(ELF_Header) + file_header.ph_num * sizeof(Program_Header)));
        stream.read((char *)new_binary_data.data(), new_binary_data.size());
        ...
        for (int i = 0; i < file_header.sh_num; i++)
        {
            ...
            new_section_headers.push_back(sh);
        }
        ...
        for (int i = 0; i < file_header.sh_num; i++)
        {
            ...
            new_section_names.push_back(std::string(name.begin(), name.end()));
        }
    }
...
ELF::createShared(std::ostream &stream)
{
    ...
    { /// Construct the ELF header
        ...
        if (file_header.ph_num == 0)
            file_header.ph_offset = 0;
        else
            file_header.ph_offset = sizeof(ELF_Header);
...
void ELF::add_section(const std::string name, const std::vector<uint8_t> data,
