...
- [Computer system description](#computer-system-description)
  ...
  - [The peripherals](#the-peripherals)
    ...
    - [The timer](#the-timer)
      - [`timer_cfg`](#timer_cfg)
  - [Example of a _user program_](#example-of-a-user-program)
...
After both a _cold_ and _warm_ restart the processor starts executing instructions beginning at address `0x40000000`.
...
2. Generates an interrupt request.
   ...
   The terminal does not _echo_ the pressed key, which means that it is not shown on the display.
   If the _user program_ does not read the value of `term_in` on time (before the next key press is detected) the previous value of `term_in` is irreversibly lost.
   Buffering of the input stream is not to be done. The processor is to be considered fast enough to guarantee (if the _user program_ is well written) the ability to read the value of `term_in` on time in the interrupt handler before it is overwritten.
...
[Line-badge]: https://img.shields.io/badge/Lines-4919-f89820?style=for-the-badge